...
def above_threshold(student_scores, threshold):
    ...
    for score in student_scores:
        if score >= threshold:
            best_students_list.append(score)
...
def letter_grades(highest):
    ...
    increment_d = 41
    increment_c = increment_d + paso
    increment_b = increment_c + paso
    increment_a = increment_b + paso

    return [increment_d, increment_c, increment_b, increment_a]
...
def student_ranking(student_scores, student_names):
    ...
    for index, name in enumerate(student_names):
        ...
        rank = index + 1
        texto_formateado = f"{rank}. {name}: {score}"
        ranking.append(texto_formateado)
